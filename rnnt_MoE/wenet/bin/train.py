...
def main():
    ...
    configs['tokenizer_conf']['special_tokens']['<sos>'] = len(symbol_table)-1
    configs['tokenizer_conf']['special_tokens']['<eos>'] = len(symbol_table)-1
    ...
    for epoch in range(start_epoch, configs.get('max_epoch', 100)):
        ...
        executor.train(model, optimizer, scheduler, train_data_loader,writer, configs, scaler, group_join,epoch)
                       #cv_data_loader, writer, configs, scaler, group_join,epoch)
