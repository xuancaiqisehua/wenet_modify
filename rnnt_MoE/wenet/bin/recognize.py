...
def main():
    ...
    for mode in args.modes:
        ...
        file_name = os.path.join(dir_name,os.path.splitext(os.path.basename(args.test_data))[0]+'.txt')
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_data_loader):
            ...
            pcm_length = batch['pcm_length']
            if 'rnnt' in args.modes[0]:
               if "rnnt_beam_search"==args.modes[0]:
                  results = fast_beam_search_one_best(
                       feats,
                       feats_lengths,
               ...
               elif 'rnnt_greedy_search'==args.modes[0]:
                   results = model.greedy_search(
                       feats,
                       feats_lengths,
                       decoding_chunk_size=args.decoding_chunk_size,
                       num_decoding_left_chunks=args.num_decoding_left_chunks,
                   ...
                   for i, key in enumerate(keys):
                      for tokens in results:
                         line = '{} {}'.format(key, ' '.join(tokenizer.detokenize(tokens)[1]).replace('@@ ',''))
               ...
               for i, key in enumerate(keys):
                   totle_length+=pcm_length[i].item()
                   for mode, hyps in results.items():
                       tokens = hyps[i].tokens
                       if 'aishell' in args.test_data:
                            line = '{} {}'.format(key, tokenizer.detokenize(tokens)[0])
                       else:
                           line = '{} {}'.format(key, ' '.join(tokenizer.detokenize(tokens)[1]).replace('@@ ','').replace('@@',''))
                           
                       logging.info('{} {}'.format(mode.ljust(max_format_len),
                                                   line))
                       files[mode].write(line + '\n')
    end = time.time()
    totle_decode_time = end-start
    RTF = totle_decode_time / (totle_length/16000)
    print(f'totle decode time {totle_decode_time}')
    print(f'totle wav_length {totle_length}')
    print(f'RTF {RTF}')
