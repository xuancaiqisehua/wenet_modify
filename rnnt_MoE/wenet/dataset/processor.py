...
def tar_file_and_group(data,lang_char):
        ...
        if not featureFile.endswith('tar'):
          ...
          for tarinfo in stream:
              ...
              with stream.extractfile(tarinfo) as file_obj:
                  try:
                      if postfix == 'txt':
                          label = file_obj.read().decode('utf8').strip().split(' ')
                          if 'common_voice_' in prefix:
                            example['label'] = [lang_char.get(i,1) for i in label]
                          else:    
                            example['label'] =[int(i) for i in label]
                          #logging.info('{} {} {}'.format(prev_prefix,example['label'],label))
        ...
        if type(sample['wav']) is np.ndarray:
          ...
          for idx in sample['label']:
              if idx == 1:
                 index+=1
          if index>3:
             logging.warning('{} more OOV word {}'.format(sample['key'],sample['label']))
             continue
