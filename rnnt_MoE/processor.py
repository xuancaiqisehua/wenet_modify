...
def tar_file_and_group(data,lang_char_de,lang_char_fr):
        ...
        if not featureFile.endswith('tar'):
          ...
          for tarinfo in stream:
              ...
              with stream.extractfile(tarinfo) as file_obj:
                  try:
                      if postfix == 'txt':
                          if 'common_voice_de' in prefix.split('/')[-1]:
                            example['label'] = [lang_char_de.get(i,'1') for i in file_obj.read().decode('utf8').strip().split(' ')]
                          elif 'common_voice_fr' in in prefix.split('/')[-1]:
                            example['label'] = [lang_char_de.get(i,'1') for i in file_obj.read().decode('utf8').strip().split(' ')]
                          else:    
                            exampl['label'] =[int(i) for i in file_obj.read().decode('utf8').strip().split(' ')]
                          print(prefix,exampl['label'])
...
def compute_fbank(data,
    ...
    for sample in data:
        ...
        if type(sample['wav']) is np.ndarray:
          ...
          yield dict(key=sample['key'], label=sample['label'], feat=mat,wav=waveform)
