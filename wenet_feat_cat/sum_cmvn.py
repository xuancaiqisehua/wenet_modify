...
def getAllCMVN(path,out_cmvn):
  ...
  for file in filelist:
    ...
    #all_mean_stat += torch.Tensor(info['mean_stat'])
    all_mean_stat += np.array(info['mean_stat'])
    #print('ori',info['mean_stat'])
    #print('Tensor',torch.Tensor(info['mean_stat']))
    #print(float(info['mean_stat'][0]),info['mean_stat'][0])
    #num+=float(info['mean_stat'][0])
    #all_var_stat += torch.Tensor(info['var_stat'])
