...
class ASRModel(torch.nn.Module):
    ...
    def forward(
        ...
        # 1. Encoder
        encoder_out_cat , encoder_out, encoder_mask = self.encoder(speech, speech_lengths)
    ...
    def _forward_encoder(
        ...
        if simulate_streaming and decoding_chunk_size > 0:
            encoder_out,_, encoder_mask = self.encoder.forward_chunk_by_chunk(
                speech,
                decoding_chunk_size=decoding_chunk_size,
                num_decoding_left_chunks=num_decoding_left_chunks
            )  # (B, maxlen, encoder_dim)
        else:
            encoder_out, _ ,encoder_mask = self.encoder(
                speech,
                speech_lengths,
                decoding_chunk_size=decoding_chunk_size,
                num_decoding_left_chunks=num_decoding_left_chunks
