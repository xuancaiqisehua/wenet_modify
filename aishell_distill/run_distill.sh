...
train_config="conf/train_16k_6_layer.yaml"
cmvn=true
dir="exp/conformer"
checkpoint="exp/conformer/359.pt"
teacher_model="teacher_model/357.pt"
teacher_config="teacher_model/train.yaml"
encoder_lr_rate=1
not_freeze_encoder_last_layers=0
kd_temperature=2.0
kd_alpha=0.2
default_weight=1.0
...
[ -f $dir/ddp_init ] && rm $dir/ddp_init
[ -f $dir/init.zip ] && rm $dir/init.zip
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  cmvn_opts=
  $cmvn && cp $trainDir/${train_set}/global_cmvn $dir
  ...
  for ((i = 0; i < $num_gpus; ++i)); do
  {
    ...
    #  --train_data $trainDir/$train_set/data.list \
    rank=`expr $node_rank \* $num_gpus + $i`
    python wenet/bin/train_distill.py --gpu $gpu_id \
      ...
      --symbol_table $dict \
      --train_data $trainDir/$train_set/data.list \
      --cv_data $trainDir/dev/data.list \
      ...
      --kd_alpha ${kd_alpha} \
      --encoder_lr_rate ${encoder_lr_rate} \
      --not_freeze_encoder_last_layers ${not_freeze_encoder_last_layers} \
      --pin_memory
  } &
  done
  wait
