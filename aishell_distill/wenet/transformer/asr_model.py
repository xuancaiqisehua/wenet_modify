...
class ASRModel(torch.nn.Module):
    ...
    def forward(
        ...
        teacher_out_attention: Optional[torch.Tensor] = None,
        teacher_out_ctc_sequence: Optional[torch.Tensor] = None,
        ...
        if self.ctc_weight != 1.0:
            student_out_attention,loss_att, acc_att = self._calc_att_loss(encoder_out, encoder_mask,
                                                    text, text_lengths)
        else:
            loss_att = None
            student_out_attention = torch.tensor(0.0)

        # 2b. CTC branch
        loss_ctc,ys_hat = self.ctc(encoder_out, encoder_out_lens, text,
                                text_lengths)
        ...
        # 3. KD
        T = temperature
        loss_kd_ctc = torch.tensor(0.0)
        loss_kd_attention = torch.tensor(0.0)

        #attention_distill
        
        if teacher_out_attention is not None and self.attention_distill and alpha > 0.0 and self.ctc_weight != 1.0:
            #size = min(student_out_attention.size(1), teacher_out_attention.size(1))
            loss_kd_attention  = self.kld_loss(F.log_softmax(student_out_attention/T, dim=-1), F.softmax(teacher_out_attention/T, dim=-1))
        # ctc序列级别损失，使用ctc_loss
        if teacher_out is not None and self.ctc_distill and alpha > 0.0 :
            #loss_kd_ctc = self.kld_loss(F.log_softmax(ys_hat[:,:size]/T, dim=2), F.softmax(teacher_out_ctc[:,:size]/T, dim=2))
            #loss_ctc_distill,_ = self.ctc(encoder_out, encoder_out_lens, teacher_out_ctc,
            #                    teacher_out_ctc.size(2))
            ### 帧级别
            loss_kd_ctc = self.kld_loss(F.log_softmax(encoder_out/T, dim=2), F.softmax(teacher_out/T, dim=2))

            ### 序列级别
            #loss_kd_ctc = self.ctc(encoder_out, encoder_out_lens,teacher_out_ctc_sequence)

        loss = loss * (1-alpha) + loss_kd_ctc * (alpha*T*T) + loss_kd_attention * (alpha*T*T)
        return {"loss": loss, "loss_att": loss_att, "loss_ctc": loss_ctc,"loss_kd_ctc": loss_kd_ctc,"loss_kd_attention" : loss_kd_attention}
    ...
    def _calc_att_loss(
        ...
        return decoder_out,loss_att, acc_att
    ...
    def forward_teacher(
        ...
        #### ctc_out
        ctc_out = self.ctc.ctc_lo(encoder_out)  # (B, maxlen, vocab_size)
        ...
        decoder_out, r_decoder_out, _ = self.decoder(encoder_out, encoder_mask,
                                                     ...
                                                     self.reverse_weight)
        ############  attention
        #top_k_logp, top_k_index = decoder_out.topk(1)
        ########################
        ...
        #return hyps, scores
        return encoder_out,decoder_out,ctc_out
