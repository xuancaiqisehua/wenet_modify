...
from wenet.utils.file_utils import read_noise
...
def _butter_bandpass_filter(data,fs,analog,low_freq=300, high_freq=3000,order=6):
  #print('data',fs,analog,low_freq,high_freq,order)
  data=data.numpy()
  nyq = 0.5 * fs
  low = low_freq / nyq
  high = high_freq / nyq
...
def tar_file_and_group(data):
    ...
    for sample in data:
        #local feature
        featureFile = sample['src']
        if not featureFile.endswith('tar'):
          for key,fbank,label in kaldi_io.read_mat_scp(featureFile):
            example = {"key":key,"label":[int(i) for i in label.split(' ')],"wav":fbank,"sample_rate":16000}
            yield example
        else:
          ...
          for tarinfo in stream:
              ...
              with stream.extractfile(tarinfo) as file_obj:
                  try:
                      if postfix == 'txt':
                          example['label'] =[int(i) for i in file_obj.read().decode('utf8').strip().split(' ')]
...
def parse_format(data):
    """ Parse key/wav/txt from json line
        ...
        Args:
            data: Iterable[str], str is a json line has 
            utt : 
    ...
    for sample in data:
        assert 'src' in sample
        arr = sample['src'].strip().split('\t') 
        if len(arr) != 7:
          continue
        key = arr[0].split(':')[1]
        txt = arr[3].split(':')[1]
        token = arr[4].split(':')[1]
        wav_file = arr[1].split(':')[1]
        ...
        try:
            ...
            example = dict(key=key,
                           txt=txt,
                           token=token,
                           label=[int(i) for i in tokenid.split(' ')],
                           wav=waveform,
...
def resample(data, resample_rate=16000):
    ...
    for sample in data:
        ...
        if type(sample['wav']) is np.ndarray:
          ...
          waveform = sample['wav']
          #while sample_rate is 8k,need to do
          if sample_rate != resample_rate:
             ...
             if resample_rate==8000:
                analog=True
                #resample 8k need bandpass
                analog_flag= random.randint(0,9)
                if analog_flag<8:
                  analog=False
                order = random.randint(3,10)
                low_freq = random.randrange(100,301,50)
                high_freq = random.randrange(3000,3501,50)
                waveform=_butter_bandpass_filter(waveform,resample_rate,analog,low_freq=low_freq,high_freq=high_freq,order=order)
...
def speed_perturb(data, speeds=None):
    """ Apply speed perturb to the data.
        ...
        Args:
            data: Iterable[{key, wav, label, sample_rate}] #wav : flaot32
            speeds(List[float]): optional speed

        Returns:
            Iterable[{key, wav, label, sample_rate}] ##wav : flaot32 
    """
    if speeds is None:
        speeds = [0.9, 1.0, 1.1]
    for sample in data:
        ...
        if type(sample['wav']) is np.ndarray:
          ...
          if speed != 1.0:
              ...
              sample['wav'] = wav
          #print('speed ',sample['wav'].dtype,sample['wav'])
          yield sample
...
def bandpass_aug(data):
    ...
    for sample in data:
        ...
        waveform=sample['wav']
        sample_rate=sample['sample_rate']
        bandpass_flag= random.randint(0,9)
        if bandpass_flag<6:
          analog_flag= random.randint(0,9)
          order = random.randint(3,10)
          low_freq = random.randrange(100,301,50)
          ...
          if analog_flag<8:
             analog=False
          waveform=_butter_bandpass_filter(waveform,sample_rate,analog,low_freq=low_freq,high_freq=high_freq,order=order)
          waveform=torch.tensor(waveform,dtype=torch.float32)
          sample['wav'] = waveform
        yield sample
          ...
          if noise_flag<6:
             ...
             if aug_type < 2:
                  ...
                  if aug_noise.any():
                    ...
                    # 叠加噪音
                    #wav = (waveform+db*noiseData)*(1<<15)  #写入音频移位操作
...
def compute_fbank(data,
    ...
    for sample in data:
        ...
        ##local feature
        if type(sample['wav']) is np.ndarray:
          yield dict(key=sample['key'], label=sample['label'],feat=torch.tensor(sample['wav'],dtype=torch.float))
        else:
          sample_rate = sample['sample_rate']
          waveform = sample['wav']
