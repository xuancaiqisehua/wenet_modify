...
decode_checkpoint=exp/paraformer/avg_20.pt
decode_checkpoint=0.pt
average_num=20
...
#decode_modes="paraformer_greedy_search paraformer_beam_search ctc_greedy_search"
decode_modes="paraformer_beam_search"
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  ...
  for mode in ${decode_modes}; do
  {
    ...
    mkdir -p $test_dir
    python3 wenet/bin/recognize.py --gpu -1 \
      --mode $mode \
      --config train.yaml \
      --data_type $data_type \
      --test_data data/test/format.lst \
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
  ...
  for mode in hlg_onebest hlg_rescore; do
  {
   ...
   python3 wenet/bin/recognize.py \
      ...
      --config $dir/train.yaml \
      --data_type $data_type \
      --test_data data/test/data.list.all \
      --checkpoint $decode_checkpoint \
      --beam_size 10 \
      --batch_size 16 \
      --penalty 0.0 \
