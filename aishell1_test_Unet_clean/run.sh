...
train_config="conf/train_conformer_Unet.yaml"
cmvn=true
dir=exp/conformer
checkpoint="exp/conformer/199.pt"
...
[ -f exp/conformer/ddp_init ] && rm exp/conformer/ddp_init
[ -f exp/conformer/init.zip ] && rm exp/conformer/init.zip
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  #init_method=tcp://10.10.50.40:23456
  ...
  for ((i = 0; i < $num_gpus; ++i)); do
  {
    ...
    python wenet/bin/train.py --gpu $gpu_id \
      ...
      --data_type $data_type \
      --symbol_table $dict \
      --train_data $trainDir/train/data.list \
