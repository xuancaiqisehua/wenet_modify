...
class ASRModel(torch.nn.Module):
    ...
    def forward(
        ...
        # 1. Encoder，这里修改了
        encoder_out_all, encoder_mask = self.encoder(speech, speech_lengths)
        encoder_out_1 = encoder_out_all[0]
        encoder_out_2 = encoder_out_all[1]
        encoder_out_3 = encoder_out_all[2]
        ...
        if self.ctc_weight != 0.0:
            loss_ctc_1 = self.ctc(encoder_out_1, encoder_out_lens, text,
                                text_lengths)
            loss_ctc_2 = self.ctc(encoder_out_2, encoder_out_lens, text,
                                text_lengths)
            loss_ctc_3 = self.ctc(encoder_out_3, encoder_out_lens, text,
                                text_lengths)
            loss_ctc_pre = loss_ctc_1 * 0.2 + loss_ctc_2 * 0.3 + loss_ctc_2 * 0.5

            loss_ctc = self.ctc(encoder_out, encoder_out_lens, text,
                                text_lengths)
            loss_ctc_final = loss_ctc_pre * 0.3 + loss_ctc * 0.7
            loss_ctc = loss_ctc_final
        else:
            loss_ctc = None
    ...
    def ctc_greedy_search(
        ...
        encoder_out, encoder_mask = self._forward_encoder(
            ...
            num_decoding_left_chunks,
            simulate_streaming)  # (B, maxlen, encoder_dim)
        encoder_out = encoder_out[-1]    # 此处是返回的sz是列表的情况，列表最后一个是最终的sz 
        maxlen = encoder_out.size(1)
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)
